...
class SystemConfig:
    ...
    def __hash__(self) -> int:
        ...
        T_C_hashable = tuple(self.T_C.flatten().tolist()) if isinstance(self.T_C, np.ndarray) else self.T_C
        ...
        M_Limit_hashable = tuple(self.M_Limit) if isinstance(self.M_Limit, list) else self.M_Limit
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            self.T_F == other.T_F and
            self.T_B == other.T_B and
            np.array_equal(self.T_C, other.T_C) and
            self.T_W == other.T_W and
            self.M_F == other.M_F and
            self.M_B == other.M_B and
            self.M_W == other.M_W and
            self.M_Limit == other.M_Limit and
            self.num_devices == other.num_devices and
            self.num_microbatches == other.num_microbatches and
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.T_C, np.ndarray):
            comm_matrix = np.ones((self.num_devices, self.num_devices)) * self.T_C - np.eye(self.num_devices) * self.T_C
