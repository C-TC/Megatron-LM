...
# Paths
BASE_PATH="$SCRATCH/cross_dc/Megatron-LM/test_crossdc/helios/multi_dc"
...
JOB_LOG_PATH="${{BASE_PATH}}/job_logs/multi_{TB_size}_{PP}_{schedule}"
...
"""
        ...
        with open(f"multi_{TB_size}_{num_dc}_{schedule}_prof.sh", "w") as f:
            f.write('\n'.join([job_str, prof_str]))
        with open(f"multi_{TB_size}_{num_dc}_{schedule}_exec.sh", "w") as f:
