...
# Paths
BASE_PATH="${{SCRATCH}}/amendment/Megatron-LM/test_crossdc/clariden/pp_dp_tradeoff"
...
CDC_ARGS=" \\
    ...
    --cdc_exp_logging \\
    --dynamic_extra_mem_factor {dynamic_extra_mem_factor} \\
...
def calculate_mem_factor(model_ratio_pp4):
    extra_mem_pp8 = (1 - model_ratio_pp4 / 2) / (1 - model_ratio_pp4) - 1
    extra_mem_pp16 = (1 - model_ratio_pp4 / 4) / (1 - model_ratio_pp4) - 1
    return [extra_mem_pp8, extra_mem_pp16]
...
for PP in [4, 8, 16]:
    gbs = mbs * DP * PP * 4
    pp_stages_per_dc = PP // num_dc
    lat_bw_as_F_stage_pairs = lat_bw_as_F_stage_pairs_dict[PP]
    num_nodes = TP * PP * DP // 4
    
    for model_mem_ratio_pp4 in [0.25, 0.5, 0.75]:
        extra_mem_factors = calculate_mem_factor(model_mem_ratio_pp4)
        if PP == 4:
            extra_mem_factor = 0.0
        elif PP == 8:
            extra_mem_factor = extra_mem_factors[0]
        elif PP == 16:
            extra_mem_factor = extra_mem_factors[1]
    
        for schedules in ['1F1B', 'ZBH1', 'ZBV', 'wave', 'ud', 'subud']:            
            if schedules in ['1F1B', 'ZBH1', 'ZBV']:
                if extra_mem_factor > 0:
                    continue
                if model_mem_ratio_pp4 != 0.25:
                    continue
            layers_per_chunk = num_layers // PP // 2 if schedules in ['ZBV', 'wave'] else num_layers // PP
            if schedules in ['1F1B', 'ZBH1', 'ZBV']:
            ...
            job_title = f'{schedules}_PP{PP}MMR{model_mem_ratio_pp4}'
            job_dict[job_title] = get_job_str(job_title, num_nodes, gbs, lat_bw_as_F_stage_pairs, layers_per_chunk, extra_cdc_args, extra_mem_factor, pp_stages_per_dc)
